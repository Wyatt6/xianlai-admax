    ...
    >
      ...
      <el-table-column label="类型" align="center" width="60px">
        <template #default="scope">
          <el-tag size="small" type="success" v-if="scope.row.type != null && scope.row.type === 'view'" round effect="plain"
            >页面</el-tag
          >
          <el-tag size="small" v-if="scope.row.type != null && scope.row.type === 'resource'" round effect="plain">资源</el-tag>
          <el-tag size="small" type="danger" v-if="scope.row.type != null && scope.row.type === 'api'" round effect="plain"
            >API</el-tag
          >
        </template>
      </el-table-column>
