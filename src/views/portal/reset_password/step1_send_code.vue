...
function updateCoutdown() {
  ...
  if (interval <= 0) {
    ...
    Storage.delete('reset_pwd_email_code_countdown_endtime')
    freezed.value = false
    return
  } else {
      ...
      Apis.common.email
        ...
        .then(result => {
          if (result && result.success) {
            ...
            Storage.set(reset_pwd_username, input.username)
            Logger.log('启动倒计时')
            endtime.value = new Date().getTime() + 60 * 1000
            countdown = setInterval(updateCoutdown, 1000)
            Storage.set('reset_pwd_email_code_countdown_endtime', endtime.value)
            freezed.value = true
          } else {
            Logger.log('发送失败')
...
onMounted(() => {
  ...
  captchaRef.value.initCaptcha(true)
  // 初始化发送验证码倒计时
  endtime.value = Storage.get('reset_pwd_email_code_countdown_endtime')
